  ...
  data() {
    return {
      buttonData: [
        ...
        { label: 'Commissioning', count: 0, path: 'allocationList',type:'CM' },
        { label: 'Preventive Maintenance', count: 0, path: 'allocationList',type:'PM' }
      ],
      chartData: {
  ...
  methods: {
    async getList() {
      ...
      try {
        ...
        const counts = serviceDetails.reduce((acc, item) => {
          if (item.reqType === 'PVM') {
            acc[0]++;
